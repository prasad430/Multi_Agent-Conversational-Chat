...
AGENT_ID = os.getenv("SPORTS_AGENT_ID", "sports-agent-1")
CERTAINTY = float(os.getenv("WEAVIATE_CERTAINTY", "0.3"))
HEARTBEAT_INTERVAL = int(os.getenv("AGENT_HEARTBEAT_SECONDS", "30"))
CLASS_NAME = "SportsNote"
BASE_URL = os.getenv("SPORTS_BASE_URL", "http://localhost:8002")
...
AGENT_CARD = {
    "id": AGENT_ID,
    "name": "Sports Agent",
    "description": "Answers sports-related queries using semantic search.",
    "base_url": BASE_URL,
    "capabilities": ["sports.stats", "sports.advice"]
}
    ...
    if query:
        try:
            ...
            if source_hits:
                ...
                insp.log_tool(agent=AGENT_ID, tool="sports.search", query=query, output=answer)
            else:
            ...
            pass
    return {"from": AGENT_ID, "tool": "sports.search", "answer": answer, "source_hits": source_hits}
